...
import { getBorderRadiusClasses, getEventColorClasses } from './lib/utils'
  ...
  const displayEnd = currentTime
    ? new Date(
        new Date(currentTime).getTime() +
          (new Date(event.dateRange.end).getTime() -
            new Date(event.dateRange.start).getTime())
      )
...
export function EventItem({
  ...
  const displayStart = useMemo(() => {
    return currentTime || new Date(event.dateRange.start)
  }, [currentTime, event.dateRange.start])
  ...
  const displayEnd = useMemo(() => {
    return currentTime
      ...
      : new Date(event.dateRange.end)
  }, [currentTime, event.dateRange.start, event.dateRange.end])
  ...
  return (
    <button
      ...
      data-past-event={isPast(new Date(event.dateRange.end)) || undefined}
      onClick={onClick}
    >
