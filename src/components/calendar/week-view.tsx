  ...
  const processedDayEvents = useMemo(() => {
    const result = days.map((day) => {
      ...
      const sortedEvents = [...dayEvents].sort((a, b) => {
        const aStart = new Date(a.dateRange.start)
        const bStart = new Date(b.dateRange.start)
        const aEnd = new Date(a.dateRange.end)
        const bEnd = new Date(b.dateRange.end)
          ...
          if (col.length === 0) {
            ...
            const overlaps = col.some((c) =>
              areIntervalsOverlapping(
                ...
                {
                  start: new Date(c.event.dateRange.start),
                  end: new Date(c.event.dateRange.end),
                }
              )
            )
            if (!overlaps) {
