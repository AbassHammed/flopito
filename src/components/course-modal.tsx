...
const CourseModal: React.FC<CourseModalProps> = ({ course, onClose }) => {
  ...
  startTime.setHours(
    Math.floor(course.start_time / 60),
    course.start_time % 60,
    0,
    0
  )

  const durationMinutes =
    course.duration || getDurationInMinutes(course.course.type)
    ...
    <Dialog open={!!course} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px]">
        ...
        <div className="grid gap-4 py-4">
          <p>
            <strong>Type:</strong> {course.course.type.substring(0, 2)}
          </p>
          <p>
            <strong>{room.type}:</strong> {room.name}
          </p>
          <p>
            <strong>Prof:</strong> {teacher.fullName}
          </p>
          <p>
            <strong>Durée:</strong> {durationHours}h
            {durationMinutes % 60 === 0 ? '' : `${durationMinutes % 60}min`}
          </p>
          <p>
            <strong>Horaire:</strong> {formatTime(startTime)} -{' '}
            {formatTime(endTime)}
          </p>
        </div>
        <DialogFooter>
