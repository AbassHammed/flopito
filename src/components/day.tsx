...
const CourseGroup = ({
  ...
  const duration =
    courses[0].duration || getDurationInMinutes(courses[0].course.type)
...
export default function Day({
  ...
  return (
    <div
      className={`relative h-full border-l ${isLast ? 'border-r' : ''} border-gray-300 dark:border-gray-700 ${isToday ? 'bg-blue-50 dark:bg-blue-950/50' : ''}`}
    >
      {Array.from({ length: 13 }, (_, i) => (
        <div
          key={i}
          className="h-16 border-t border-b border-gray-300 dark:border-gray-700"
        />
      ))}
