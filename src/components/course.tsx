...
export default function Course({
  ...
  const amphiBorderStyle = isAmphi
    ? 'border-solid border-2 dark:border-white border-black'
    : ''
  ...
  const halfGroupLabel =
    selectedYear === 'BUT1' && isHalfGroupCourse && !isAmphi ? groupName : null
  ...
  return (
    ...
    >
      ...
      {halfGroupLabel && (
        <span
          className={`absolute top-2 left-2 font-bold ${view === 'week' ? 'text-[10px] sm:text-xs' : 'text-xs'}`}
        >
          {halfGroupLabel}
        </span>
      )}
      {view === 'day' ? (
        ...
        <>
          <div className="text-sm font-bold tracking-tight leading-tight hidden sm:block">
            {course.course.module.name}
          </div>
      ...
      <div className={view === 'week' ? 'text-[10px] sm:text-xs' : 'text-xs'}>
        <span className={view === 'week' ? 'hidden sm:inline' : ''}>
          {room.type}:{' '}
        </span>
        {room.name}
      </div>
